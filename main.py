...
multicast_test_path = Path('./multicast/multicast.py')
...
def exec_linux_(command: str) -> Union[None, bool]:
    ...
    if command[0] == 'mt':
        if len(command) == 1:
            cmd = f'gnome-terminal -- python3 {multithread_server_path}'
            subprocess.Popen(cmd.split(), stdout=subprocess.PIPE)
            time.sleep(2)
            cmd = f'gnome-terminal -- python3 {multithread_test_path}'
            subprocess.Popen(cmd.split(), stdout=subprocess.PIPE)
        elif len(command) == 2:
            if command[1] == 't':
                cmd = f'gnome-terminal -- python3 {multithread_test_path}'
        ...
        if len(command) == 1:
            cmd = f'gnome-terminal -- python3 {multicast_test_path}'
            subprocess.Popen(cmd.split(), stdout=subprocess.PIPE)
    else:
        return False
